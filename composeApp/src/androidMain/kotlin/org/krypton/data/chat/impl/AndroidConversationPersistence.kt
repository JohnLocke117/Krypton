...
/**
 * Android implementation of ConversationRepository using standard File APIs.
 * 
 ...
 */
class AndroidConversationPersistence(
    private val context: Context
) : org.krypton.chat.conversation.ConversationRepository {
    ...
    override suspend fun getConversationMessages(conversationId: ConversationId): List<ChatMessage> = withContext(Dispatchers.IO) {
        try {
            ...
            AppLogger.e("AndroidConversationPersistence", "Failed to load messages for conversation: ${conversationId.value}", e)
            emptyList()
        }
    }
    ...
    override suspend fun appendMessage(message: ChatMessage) = withContext(Dispatchers.IO) {
        try {
            ...
            AppLogger.e("AndroidConversationPersistence", "Failed to append message to conversation: ${message.conversationId.value}", e)
            throw e
        }
    }
    ...
    override suspend fun updateConversationSummary(
        ...
        try {
            ...
            AppLogger.e("AndroidConversationPersistence", "Failed to update conversation summary: ${conversationId.value}", e)
        }
    }
    ...
    override suspend fun deleteConversation(conversationId: ConversationId) = withContext(Dispatchers.IO) {
        try {
            ...
            AppLogger.e("AndroidConversationPersistence", "Failed to delete conversation: ${conversationId.value}", e)
            throw e
        }
    }
    ...
    private fun loadConversationIndex(vaultId: String): List<ConversationSummary> {
        return try {
            ...
            if (indexPath.exists() && indexPath.isFile) {
                val content = indexPath.readText()
                if (content.isBlank()) {
                    emptyList()
                } else {
                    val index = json.decodeFromString<ConversationIndex>(content)
                    index.conversations.sortedByDescending { it.updatedAt }
                }
            } else {
                emptyList()
            }
        } catch (e: Exception) {
            AppLogger.e("AndroidConversationPersistence", "Failed to load conversation index for vault: $vaultId", e)
            emptyList()
        }
    }
    ...
    private fun loadConversationData(vaultId: String, conversationId: ConversationId): ConversationData? {
        return try {
            ...
            AppLogger.e("AndroidConversationPersistence", "Failed to load conversation data: ${conversationId.value}", e)
            null
        }
    }
    ...
    private fun saveConversationData(vaultId: String, data: ConversationData) {
        ...
        conversationFile.parentFile?.mkdirs()
        
        val content = json.encodeToString(ConversationData.serializer(), data)
        conversationFile.writeText(content)
    }
    
    private fun getChatDirectory(vaultId: String): File {
        // For "default" vault ID (when no vault is open), use app's external files directory
        if (vaultId == "default") {
            val externalDir = context.getExternalFilesDir(null)
            return File(externalDir, ".krypton/chat/default")
        }
        val vaultFile = File(vaultId)
        return File(vaultFile, ".krypton/chat")
    }
    
    private fun getIndexPath(vaultId: String): File {
        return File(getChatDirectory(vaultId), "index.json")
    }
    
    private fun getConversationFilePath(vaultId: String, conversationId: ConversationId): File {
        return File(getChatDirectory(vaultId), "${conversationId.value}.json")
    }
    
    private fun findVaultIdForConversation(conversationId: ConversationId): String? {
        // Load from registry if it exists
        val registryFile = getRegistryFile()
        if (registryFile.exists()) {
            try {
                ...
                AppLogger.e("AndroidConversationPersistence", "Failed to load conversation registry", e)
            }
        }
        
        // Fallback: search through app's external files directory for .krypton folders
        // This is a simplified search - in production you might want a better approach
        val externalDir = context.getExternalFilesDir(null)
        if (externalDir != null) {
            try {
                externalDir.walkTopDown().maxDepth(5).forEach { file ->
                    if (file.name == ".krypton" && file.isDirectory) {
                        val chatDir = File(file, "chat")
                        if (chatDir.exists()) {
                            val conversationFile = File(chatDir, "${conversationId.value}.json")
                            if (conversationFile.exists()) {
                                // Extract vaultId from path: .../vault/.krypton/chat/...
                                val vaultPath = file.parent
                                return vaultPath
                            }
                        }
                    }
                ...
                AppLogger.e("AndroidConversationPersistence", "Failed to search for conversation", e)
            }
        }
    ...
    private fun getRegistryFile(): File {
        val externalDir = context.getExternalFilesDir(null)
        val kryptonDir = File(externalDir, ".krypton")
        return File(kryptonDir, "conversation-registry.json")
    }
    ...
    private fun updateRegistry(conversationId: ConversationId, vaultId: String) {
        try {
            ...
            val existingRegistry = if (registryFile.exists()) {
                try {
                    val content = registryFile.readText()
                    if (content.isNotBlank()) {
            ...
            AppLogger.e("AndroidConversationPersistence", "Failed to update conversation registry", e)
        }
    }
