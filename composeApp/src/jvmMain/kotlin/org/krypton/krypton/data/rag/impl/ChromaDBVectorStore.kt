...
class ChromaDBVectorStore(
     ...
     * 
     * @param chunks List of NoteChunk objects to store
     * @param vaultPath Absolute path of the vault
     * @param fileHash SHA-256 hash of the file content
     */
    suspend fun upsertWithMetadata(chunks: List<NoteChunk>, vaultPath: String, fileHash: String) = withContext(Dispatchers.IO) {
        if (chunks.isEmpty()) return@withContext
        ...
        try {
            ...
            val metadatas = chunks.map { chunk ->
                buildJsonObject {
                    ...
                    put("file_path", JsonPrimitive(chunk.filePath))
