    ...
    override suspend fun sendMessage(
        ...
        while (retryCount <= maxRetries) {
            try {
                ...
                return@withContext history + userMsg + assistantMsg
                ...
                if (retryCount <= maxRetries) {
                    ...
                    AppLogger.w("Chat", "Chat request failed (attempt $retryCount/$maxRetries), retrying in ${delayMs}ms: ${e.message}", e)
                    delay(delayMs)
                } else {
                    AppLogger.e("Chat", "Chat request failed after $maxRetries retries: ${e.message}", e)
                    throw e
                }
            } catch (e: Exception) {
                lastException = e
                ...
                if (retryCount <= maxRetries) {
                    ...
                    val errorMsg = "Could not reach Ollama after $maxRetries retries. Please make sure `ollama serve` is running. Error: ${e.message}"
                    AppLogger.e("Chat", errorMsg, e)
                    throw ChatServiceException(errorMsg, e)
                }
            }
        }
        
        // Should never reach here, but handle it just in case
        throw lastException ?: ChatServiceException("Unknown error in chat service")
    }
