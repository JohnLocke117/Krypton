...
/**
 * Gemini embedding API client for JVM.
 * 
 * Uses Gemini's embedding API to generate embeddings for both queries and documents.
 * 
 ...
 * @param outputDimension Output dimension for embeddings (default: 768 to match common collections)
...
class GeminiEmbedder(
    ...
    private suspend fun embedText(text: String): Embedding = withContext(Dispatchers.IO) {
        AppLogger.d("GeminiEmbedder", "Embedding text (length=${text.length}, requested_dimension=$outputDimension)")
    ...
    override suspend fun embedQuery(text: String): Embedding = embedText(text)
    
    override suspend fun embedDocument(texts: List<String>): List<Embedding> = withContext(Dispatchers.IO) {
        // Embed each document text individually
    ...
    override suspend fun embed(texts: List<String>, taskType: org.krypton.rag.EmbeddingTaskType): List<FloatArray> {
        return when (taskType) {
            org.krypton.rag.EmbeddingTaskType.SEARCH_DOCUMENT -> {
                embedDocument(texts).map { it.vector.toFloatArray() }
            }
